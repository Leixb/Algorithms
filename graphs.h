...
class Adj {
    ...
    vector <T>& operator[] (const size_t& p) {
        if (p < v.size()) return v[p];
        else throw error::INV_MEM;
    }
    T& operator[] (const edge& e) {
        if(e.a < v.size() and e.b < v.size()) return v[e.a][e.b];
        else throw error::INV_MEM;
    }
    ...
    Adj kruskal () const {
        ...
        while (!q.empty()) {
            edge ed = q.top(); q.pop();
            if (c + 1 == v.size()) return mat;
            if (ed.w != 0 and !mat.dfs(ed.a, ed.b)) {
                mat[ed.a][ed.b] = ed.w;
